...
flowchart LR
    ...
    linkStyle default stroke:#EAEAEA,stroke-width:3px;
```
## CloudWatch Alarm
...
AWS Systems Manager Automation is a managed way to run **repeatable operational tasks** by using **runbooks**.

A runbook is an SSM document made of steps. It can restart EC2 instances, create AMIs, patch things, collect information, or fix problems automatically.

### Key Use Case
Use it when you want **safe, repeatable, multi-step operations** on AWS resources or managed instances.

### Practical Scenario
A company wants to fix noncompliant EC2 settings automatically.

A Config rule finds the issue, then a Systems Manager Automation runbook runs the remediation steps without an engineer doing each step manually.

### Exam Tip / Trigger
Look for words like **runbook**, **multi-step remediation**, **operational task**, **automated fix**, **predefined document**, or **Systems Manager document**.

A common exam clue is: **“automate operational steps across AWS resources”**.

Trap: if the question is only about running a command or script on many EC2 instances, **Run Command** may be the better answer.

### Difference Comparison
Compared with **Systems Manager Run Command**:

- **Automation** = multi-step workflow using a runbook, often for remediation and operations on AWS resources
- **Run Command** = run commands on managed nodes

For the exam, choose **Automation** when the workflow is more than just “run this command.”

### Memory Hook
**Automation = runbook for repeatable ops and fixes.**

### Mermaid Diagram
```mermaid
flowchart LR
    A[🧩 AWS Resource / Issue]:::data --> B[📘 Automation Runbook]:::core
    B --> C[⚙️ Step 1]:::app
    C --> D[⚙️ Step 2]:::app
    D --> E[✅ Remediated / Updated State]:::dash
...
| AWS Systems Manager Automation | A managed runbook-based service for repeatable operational workflows | Multi-step remediation and operational tasks on AWS resources or managed instances | Often confused with Systems Manager Run Command | Runbook, remediation, multi-step ops, automated fix | Runbook for repeatable ops and fixes |